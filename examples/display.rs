       ...
       let _ = board.activity_led.set_high();
        ...
        loop {
            ...
            if avail > 0 {
                ...
                let src =
                    if flipper == 0 { unsafe { &mut FAST_IMG0 } } else { unsafe { &mut FAST_IMG1 } };
                flipper = (flipper + 1) % 2;

                convert_to_one_bpp(src, &mut one_bit_buf);

                let snap_raw: ImageRaw<BinaryColor> = ImageRaw::new(&one_bit_buf, 64, 64);
                let snap_img = Image::new(&snap_raw, Point::new(32, 0));

                let _ = board.activity_led.toggle();
                if snap_img.draw(&mut disp).is_ok() {
                    let _ = disp.flush();
                }
            }
            else {
...
fn convert_to_one_bpp(src: &[u8], dst: &mut [u8]) {
    for y in 0..64 {
        for x  in 0..64 {
            let src_idx = (x + y*64) as usize;
            let dst_idx = src_idx / 8;
            let mut dst_val = dst[dst_idx];
